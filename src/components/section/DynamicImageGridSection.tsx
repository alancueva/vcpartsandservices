...
import Image from 'next/image'; // Se comenta para usar img y evitar errores de configuración en este entorno
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <header className="mb-12">
                    <h2 className="text-5xl font-extrabold text-gray-900 tracking-tight mb-2">
                        {title} <span className={`italic font-serif ${highlightColor}`}>{highlightedWord}</span>
                    </h2>
                    {/* Línea decorativa que toma el color del resalte */}
                    <div className={`w-16 h-1 ${highlightColor.replace('text-', 'bg-')} mx-auto rounded`}></div>
                </header>
                        ...
                        {images.map((image, index) => (
                            ...
                            >
                                <div className="relative w-full aspect-square overflow-hidden rounded-lg">
                                    {/* En un entorno Next.js real, usaríamos <Image> */}
                                    <Image
                                        src={image.src}
                                        alt={image.alt}
                                        width={500}
